...
为了满足您的需求，我将修改之前的 `install_arch.sh` 脚本，移除 SDDM 登录管理器（Display Manager）相关内容，并配置 Hyprland 为手动启动方式。以下是具体更改：

1. **移除 SDDM**：
   - 删除 `install_sddm` 函数和相关调用。
   - 从 `pacstrap` 中移除 `sddm` 包。
2. **手动启动 Hyprland**：
   - 在 `configure_system` 函数中，为用户 `syaofox` 的 `~/.bashrc` 添加命令，允许通过终端手动启动 Hyprland（`Hyprland` 命令）。
   - 添加提示，指导用户登录后如何启动 Hyprland。
3. **保留桌面管理工具**：
   - 保留通知（`mako`）、音量（`pamixer` + `swayosd`）、截图（`grim` + `slurp` + `hyprshot`）工具及其配置。
4. **其他**：
   - 保留时间同步（`systemd-timesyncd`）、NVIDIA 配置（`nvidia-open-dkms`）、Btrfs 和 Snapper 设置。
   - 保持交互式菜单（`dialog`）和错误检查。

### 优化后的安装脚本

<xaiArtifact artifact_id="df120ad5-5b28-4f1d-9377-19ad8e96ca68" artifact_version_id="b64b2584-ae72-40d7-b007-3b555473fe9f" title="install_arch.sh" contentType="text/x-sh">
#!/bin/bash

# Arch Linux Btrfs 开发者定制安装脚本（含 Hyprland、NVIDIA 和桌面管理工具）
# 基于 2025 年 10 月 23 日的 Arch Linux Wiki 和 Hyprland Wiki
# 优化：交互式菜单（dialog），模块化设计，手动启动 Hyprland，包含通知、音量、截图工具
...
install_system() {
    ...
    dialog --yesno "是否安装桌面管理工具（通知：mako，音量：pamixer+swayosd，截图：grim+slurp+hyprshot）？" 8 50
...
exit
EOF
    dialog --msgbox "系统配置完成。\n登录后输入 'Hyprland' 启动桌面环境。" 8 40
    echo -e "${GREEN}系统配置完成。${NC}" | tee -a "$LOGFILE"
}

# 安装 paru、snapper、brave 和 hyprshot
install_extras() {
    echo "安装 paru、snapper、brave 和 hyprshot..." | tee -a "$LOGFILE"
    arch-chroot /mnt /bin/bash -c "
        su syaofox -c '
            ...
            paru -S --noconfirm snapper snap-pac brave-bin hyprshot
        '
        ...
        '
    "
    dialog --msgbox "paru、snapper、brave 和 hyprshot 安装完成。" 6 30
    echo -e "${GREEN}paru、snapper、brave 和 hyprshot 安装完成。${NC}" | tee -a "$LOGFILE"
}
...
main() {
    ...
    install_extras
    final_check
    dialog --msgbox "安装完成！请拔掉 USB 启动盘并点击 OK 重启。\n登录后输入 'Hyprland' 启动桌面环境。" 10 50
    umount -R /mnt
    reboot
}
...
### 更新说明
1. **移除 SDDM**：
   - 删除 `install_sddm` 函数及其在 `main` 函数中的调用。
   - 从 `install_system` 的 `pacstrap` 命令中移除 `sddm` 包。
2. **手动启动 Hyprland**：
   - 在 `configure_system` 中，为用户 `syaofox` 的 `~/.bashrc` 添加提示：
     ```bash
     echo "输入 'Hyprland' 启动桌面环境"
     ```
   - 在 `configure_system` 和 `main` 的最后，添加 `dialog` 提示，提醒用户登录后输入 `Hyprland` 启动桌面。
3. **保留功能**：
   - 通知（`mako`）、音量（`pamixer` + `swayosd`）、截图（`grim` + `slurp` + `hyprshot`）工具及其绑定。
   - 时间同步（`systemd-timesyncd`）和国内时间服务器配置。
   - NVIDIA 配置（`nvidia-open-dkms`）、Btrfs 子卷、Snapper 快照。
   - 交互式菜单（`dialog`）和错误检查。
4. **日志和提示**：
   - 日志记录到 `/root/install_arch.log`。
   - 最终提示明确说明手动启动 Hyprland 的方法。
...
- **终端登录**：无 SDDM 时，系统启动到终端，需手动输入用户名/密码和 `Hyprland`。

如需进一步定制（如添加其他工具、调整绑定或配置其他启动方式），请提供详细信息！