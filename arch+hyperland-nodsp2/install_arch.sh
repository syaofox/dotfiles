...
# Arch Linux Btrfs 开发者定制安装脚本（含 Hyprland、NVIDIA、Alacritty 和 Rofi）
# 基于 2025 年 10 月 23 日的 Arch Linux Wiki 和 Hyprland Wiki
# 优化：交互式菜单（dialog），模块化设计，手动启动 Hyprland，Alacritty (Super+Enter)，Rofi (Super+Space)
...
install_system() {
    ...
    pacstrap /mnt base linux linux-headers systemd vim networkmanager \
        ...
        $IWD $DESKTOP_TOOLS $ALACRITTY $ROFI
...
# 配置 Hyprland、Fcitx5、Alacritty 和 Rofi
...
exit
EOF
    dialog --msgbox "系统配置完成。\n登录后输入 'Hyprland' 启动桌面环境。\nSuper + Enter: 打开 Alacritty\nSuper + Space: 打开 Rofi" 10 50
    echo -e "${GREEN}系统配置完成。${NC}" | tee -a "$LOGFILE"
}

# 安装 paru、snapper、brave、hyprshot 和 rofi
install_extras() {
    echo "安装 paru、snapper、brave、hyprshot 和 rofi..." | tee -a "$LOGFILE"
    arch-chroot /mnt /bin/bash -c "
        su syaofox -c '
            ...
            paru -S --noconfirm snapper snap-pac brave-bin hyprshot rofi
        '
        ...
        '
    "
    dialog --msgbox "paru、snapper、brave、hyprshot 和 rofi 安装完成。" 6 30
    echo -e "${GREEN}paru、snapper、brave、hyprshot 和 rofi 安装完成。${NC}" | tee -a "$LOGFILE"
}
